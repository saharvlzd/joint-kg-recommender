...
Original file is located at
    https://colab.research.google.com/drive/1UeOTz5Lj66OfEES8AghWgP3jZc0IbRxJ
"""
...
from jTransUP.utils.misc import to_gpu, projection_transR_pytorch, projection_transR_pytorch_batch, projection_transH_pytorch
from jTransUP.models.transR import TransRModel
...
class jTransUPModel(nn.Module):
    def __init__(self,
        ...
        pref_proj_weight = torch.FloatTensor(self.rel_total, self.embedding_size* self.embedding_size)
        nn.init.xavier_uniform(user_weight)
        nn.init.xavier_uniform(item_weight)
        nn.init.xavier_uniform(pref_weight)

        if self.is_pretrained:
            nn.init.eye(pref_proj_weight)
            pref_proj_weight = pref_proj_weight.view(-1).expand(self.relation_total, -1)
        else:
            nn.init.xavier_uniform(pref_proj_weight)
        ...
        self.pref_proj_embeddings = nn.Embedding(self.rel_total, self.embedding_size* self.embedding_size)
        ...
        normalize_pref_proj_emb = F.normalize(self.pref_proj_embeddings.weight.data, p=2, dim=1)
        self.pref_embeddings.weight.data = normalize_pref_emb
        self.pref_proj_embeddings.weight.data = normalize_pref_proj_emb
    ...
    def forward(self, ratings, triples, is_rec=True):
        ...
        if is_rec and ratings is not None:
            ...
            proj_u_e = projection_transR_pytorch(u_e, proj)
            proj_i_e = projection_transR_pytorch(ie_e, proj)
            proj_h_e=torch.ones((512,8))
            proj_t_e=torch.ones((512,8))
            ...
            proj_u_e=torch.ones((512,8))
            proj_i_e=torch.ones((512,8))
            if self.L1_flag:
    ...
    def evaluateRec(self, u_ids, all_i_ids=None):
        ...
        #print("all_i",all_i.shape)
        u = self.user_embeddings(u_ids)
        #print("u",u.shape)
        #proj_e = self.proj_embeddings(all_i_ids)
        # expand u and i to pair wise match, batch * item * dim 
        u_e1 = u.expand(item_total, batch_size, dim).permute(1, 0, 2)
        u_e=u_e1[0,:, :]
        #print("u_e",u_e.shape)
        ...
        ie_e1 = i_e + e_e
        ie_e=ie_e1[0,:, :]
        #print("ie_e",ie_e.shape)
        #ie=all_i+self.ent_embeddings(e_var)
        #print("********************")
        #print("&&&",ie.shape)
       # print(self.ent_embeddings(e_var).shape)
        
        #print(u.shape)
        #print(ie.shape)
        # batch * item * dim
        _, r_e, proj = self.getPreferences(u_e, ie_e, use_st_gumbel=self.use_st_gumbel)
        #_, r_e, proj1 = self.getPreferences(u, ie, use_st_gumbel=self.use_st_gumbel)
        ...
        proj_u_e = projection_transR_pytorch_batch(u_e, proj)
        proj_i_e = projection_transR_pytorch_batch(ie_e, proj)
        #print("%%%%%%%%%%")
        #print(proj_i_e.shape)
        # batch * item
        if self.L1_flag:
    ...
    def getPreferences(self, u_e, i_e, use_st_gumbel=False):
        ...
        proj = torch.matmul(pre_probs, self.pref_proj_embeddings.weight + self.proj_embeddings.weight) / 2

        return pre_probs, r_e, proj
