...
def train_loop(FLAGS, model, trainer, rating_train_dataset, triple_train_dataset, rating_eval_datasets, triple_eval_datasets, e_map, i_map, ikg_map, logger, vis=None, is_report=False):
    ...
    for _ in range(trainer.step, FLAGS.training_steps):
        ...
        if trainer.step % FLAGS.eval_interval_steps == 0 and (len(rating_eval_datasets) > 0 or len(triple_eval_datasets)>0):
            ...
            #print("<___________________rating_eval_datasets_____________>",rating_eval_datasets)
            rec_performances = []
            for i, eval_data in enumerate(rating_eval_datasets):
                ...
                #print("<___________________i_____________> ",i)
                
                #print("<___________________eval_data0_____________> ",eval_data[0])
                #print("<___________________eval_data1_____________> ",eval_data[1])
                #print("<___________________eval_data2_____________> ",eval_data[2])
                #print("<___________________eval_data3_____________> ",eval_data[3])
                #print("<___________________all_eval_dicts_____________> ",all_eval_dicts)
            ...
            for i, eval_data in enumerate(triple_eval_datasets):
                ...
                #print("<___________________eval_data2_____________> ",eval_data[2])
                #print("<___________________eval_data3_____________> ",eval_data[3])
                #("<___________________eval_dat4_____________> ",eval_data[4])
               #print("<___________________eval_data5_____________> ",eval_data[5])
        ...
        if trainer.step % 10 < step_to_switch :
           ...
           # pos_score ,aaaa,bbbb,proj_u_e,proj_i_e = model( (u_var, pi_var), None, is_rec=True)

            # Run model. output: batch_size * cand_num, input: ratings, triples, is_rec=True
            pos_score ,aaaa,bbbb,proj_u_e,proj_i_e  = model( (u_var, pi_var), None, is_rec=True)
            neg_score ,aaaa,bbbb,proj_u_e,proj_i_e   = model( (u_var, ni_var), None, is_rec=True)

            # Calculate loss.
            losses = bprLoss(pos_score, neg_score, target=trainer.model_target)
            
            #losses = loss.marginLoss()(pos_score, neg_score, FLAGS.margin)
            user_embeddings = model.user_embeddings(torch.cat([u_var]))
            item_embeddings = model.item_embeddings(torch.cat([pi_var,ni_var]))
            #if FLAGS.model_type in ["transup", "jtransup"]:
             #  losses += orthogonalLoss(model.pref_embeddings.weight, model.pref_proj_embeddings.weight)
            #print(" user_embeddings", user_embeddings.shape)
            #print(" item_embeddings", item_embeddings.shape)
            #losses = losses + loss.normLoss(model.user_embeddings.weight) +loss.normLoss(model.item_embeddings.weight)+ loss.normLoss(model.pref_embeddings.weight)
            losses = losses + loss.normLoss(user_embeddings) +loss.normLoss(item_embeddings)+ loss.normLoss(model.pref_embeddings.weight)+loss.normLoss(proj_u_e)+loss.normLoss(proj_i_e)
        # kg train
        else :
            ...
            pos_score ,proj_h_e,proj_t_e,aaaa,bbbb = model(None, (ph_var, pt_var, pr_var), is_rec=False)
            neg_score ,proj_h_e,proj_t_e,aaaa,bbbb  = model(None, (nh_var, nt_var, nr_var), is_rec=False)
            ...
            rel_embeddings = model.rel_embeddings(torch.cat([pr_var, nr_var]))
               ...
               # norm_embeddings = model.proj_embeddings(torch.cat([pr_var, nr_var]))
                #losses += loss.orthogonalLoss(rel_embeddings, norm_embeddings)
            losses = losses + loss.normLoss(ent_embeddings) + loss.normLoss(rel_embeddings)+loss.normLoss(proj_t_e)+loss.normLoss(proj_h_e)
